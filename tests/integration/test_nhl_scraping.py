"""Integration tests for live NHL scraping
...
@pytest.mark.network
@pytest.mark.asyncio
class TestNHLLiveScraping:
    """Test live scraping of plaintextsports.com for NHL"""

    async def test_fetch_nhl_games_live(self, integration_bot):
        """Test fetching real NHL games from plaintextsports.com"""
        cog = NHLGames(integration_bot)
...
@pytest.mark.network
@pytest.mark.asyncio
class TestNHLCommandOutput:
    """Test and display simulated NHL command output"""

    async def test_nhl_games_command_output(self, integration_bot):
        """Simulate !nhl and print what the embeds would show"""
        cog = NHLGames(integration_bot)
    ...
    async def test_nhl_team_command_output(self, integration_bot):
        """Simulate !nhl team bruins and print what the embed would show"""
        cog = NHLGames(integration_bot)
        ...
        try:
            ...
            team_slug, full_name = cog.find_team_slug("bruins", teams)

            if not team_slug:
                print("\n[SKIP] Could not find Bruins in team list")
                return
            ...
            print_embed(embed, "!nhl team bruins")
