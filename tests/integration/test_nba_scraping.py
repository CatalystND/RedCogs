"""Integration tests for live NBA scraping
...
@pytest.mark.network
@pytest.mark.asyncio
class TestNBALiveScraping:
    """Test live scraping of plaintextsports.com for NBA"""

    async def test_fetch_nba_games_live(self, integration_bot):
        """Test fetching real NBA games from plaintextsports.com"""
        cog = NBAGames(integration_bot)
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                # String error message is acceptable
...
@pytest.mark.network
@pytest.mark.asyncio
class TestNBACommandOutput:
    """Test and display simulated NBA command output"""

    async def test_nba_games_command_output(self, integration_bot):
        """Simulate !nba and print what the embeds would show"""
        cog = NBAGames(integration_bot)
    ...
    async def test_nba_team_command_output(self, integration_bot):
        """Simulate !nba team lakers and print what the embed would show"""
        cog = NBAGames(integration_bot)
        ...
        try:
            ...
            team_slug, full_name = cog.find_team_slug("lakers", teams)

            if not team_slug:
                print("\n[SKIP] Could not find Lakers in team list")
                return
            ...
            print_embed(embed, "!nba team lakers")
